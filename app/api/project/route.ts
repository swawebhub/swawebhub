...
export async function POST(request: Request) {
  try {
    ...
    const serviceLabel = typeof service === "string" ? service : String(service);

    const { data, error } = await resend.emails.send({
      from: "SWAWEBHUB <onboarding@resend.dev>",
      to: [TO],
      replyTo: email,
      subject: `New Project Request - ${serviceLabel}`,
      html: `
        <div style="font-family: Arial, sans-serif; line-height: 1.5; max-width: 600px; margin: 0 auto;">
          <h2 style="color: #1C3F09; margin-bottom: 16px;">New Project Request</h2>
          ...
          <div style="background: #f5f5f5; padding: 16px; border-radius: 8px; margin-top: 16px;">
            <p style="margin: 0 0 8px 0; font-weight: bold;">Project Details:</p>
            <p style="margin: 0; white-space: pre-wrap;">${escapeHtml(message || "N/A")}</p>
          </div>
        </div>
      `,
    });
    ...
    if (error) {
      ...
      return NextResponse.json({ ok: false, error: error.message || "Failed to submit" }, { status: 500 });
    }
